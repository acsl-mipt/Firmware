...
public:
    ...
    static void setLogSpamming(bool spamming);
    static void temporaryLog(const char *fmt);
    template<typename T>
    static void temporaryLog(const char *fmt, T argument);
...
private:
    ...
    static bool _logSpamming;
};

template<typename T>
void BlockLocalPositionEstimator::temporaryLog(const char* fmt, T argument)
{
    if(_logSpamming)
        PX4_TEMPWARN(fmt, argument);
}
